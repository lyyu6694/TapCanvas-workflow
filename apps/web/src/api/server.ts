...
export async function sendVerificationCode(email: string): Promise<{ success: boolean; error?: string }> {
  ...
  if (!r.ok) {
    return { success: false, error: data?.error || `发送验证码失败: ${r.status}` }
  }
  return { success: true }
}
...
export async function verifyEmailCode(
  ...
  if (!r.ok || !data.success) {
    return { success: false, error: data?.error || `验证失败: ${r.status}` }
  }
...
// Invitation code management (admin only)
...
export async function generateInvitationCode(expiresInDays?: number): Promise<{ success: boolean; code?: string; id?: string; error?: string }> {
  const r = await apiFetch(`${API_BASE}/auth/invitation/generate`, withAuth({
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const data = await r.json().catch(() => ({}))
  if (!r.ok || !data.success) {
    return { success: false, error: data?.error || `生成邀请码失败: ${r.status}` }
  }
  return { success: true, code: data.code, id: data.id }
}

export async function listInvitationCodes(): Promise<{ success: boolean; codes?: InvitationCodeDto[]; error?: string }> {
  const r = await apiFetch(`${API_BASE}/auth/invitation/list`, withAuth())
  const data = await r.json().catch(() => ({}))
  if (!r.ok || !data.success) {
    return { success: false, error: data?.error || `获取邀请码列表失败: ${r.status}` }
  }
